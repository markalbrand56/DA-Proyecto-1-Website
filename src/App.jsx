...
function App() {
  ...
  const [turingData, setTuringData] = useState(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://tc-proyecto-3-markalbrand56.koyeb.app/turingMachine/${string}`);
      if (!response.ok) {
  ...
  return (
    <div className="flex flex-col w-screen h-screen justify-center items-center bg-gray-100">
      ...
      <button className='h-10 w-10 text-white bg-blue-500 rounded-full fixed bottom-0 right-0 m-4 p-2 hover:bg-blue-700' onClick={() => setShowMachine(!showMachine)}>
      ...
      )}
      {
        showAnswer && !showMachine && (
          <div className='flex flex-col w-1/2 h-1/2 justify-start items-center bg-blue-600 rounded-md'>
          <span className='text-xl text-white font-bold p-4 uppercase'>Resultado</span>
          {loading && <p>Cargando...</p>}
          {turingData && <TuringTape steps={turingData.resultado} />}
          </div>
        )
      }

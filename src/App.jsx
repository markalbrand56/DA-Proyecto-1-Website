...
function App() {
  ...
  return (
    <div className="flex flex-col w-screen h-screen justify-center items-center bg-gray-100">
      <h1 className='text-4xl text-blue-500 font-bold m-0'> PANAS </h1>
      <h1 className='text-4xl text-black font-bold'> Turing Machine <span className='text-blue-500'>Simulator</span></h1>
      {!showMachine && !showAnswer && (<form className='flex flex-col w-1/2 h-1/2 justify-center items-center bg-gray-100'>
            <label htmlFor="tape" className='text-black'>Cadena</label>
            <input type="text" id="tape" name="tape" className='w-1/2 bg-black p-2 rounded-md'></input>
            <button type="submit" value="run" className='w-1/2 text-white bg-blue-500 my-8 rounded-md p-2' onClick={onClickshowAnswer}>Run</button>
      </form>)}
      <button className='h-10 w-10 text-white bg-blue-500 rounded-full fixed bottom-0 right-0 m-4 p-2 hover:bg-blue-700' onClick={onClickMachine}>
      ...
      {showMachine && !showAnswer && (
        <div className='flex flex-col w-1/2 h-1/2 justify-center items-center bg-gray-600 rounded-md'>
        <span className='text-xl text-white font-bold'>aca va la maquinaaaaa</span>
        </div>
      )}
      {
        showAnswer && !showMachine && (
          <div className='flex flex-col w-1/2 h-1/2 justify-start items-center bg-gray-600 rounded-md'>
          <span className='text-xl text-white font-bold'>aca va la respuesta</span>
          <TuringTape steps={steps} />
          </div>
        )
      }

...
function App() {
  ...
  const fetchMachine = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://tc-proyecto-3-markalbrand56.koyeb.app/json`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
  ...
  const onClickshowMachine = async () => {
    fetchMachine();
  ...
  return (
    <div className="flex flex-col w-screen h-screen justify-center items-center bg-gray-100">
      ...
      {showMachine && !showAnswer && (
        <div className='machine flex flex-col w-1/2 h-1/2 justify-start items-center bg-gray-600 rounded-md p-4'>
